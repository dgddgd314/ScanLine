...
from ui.style_config import OverlayStyle
...
class ScanLineUI(QWidget):
    ...
    @pyqtSlot(bool)
    def set_scanning_state(self, state):
    ...
    def paintEvent(self, event):
        ...
        if self.is_scanning:
            bg_color = OverlayStyle.SCAN_BG_COLOR
            line_color = OverlayStyle.SCAN_LINE_COLOR
            border_color = OverlayStyle.SCAN_BORDER_COLOR
        else:
            bg_color = OverlayStyle.IDLE_BG_COLOR
            line_color = OverlayStyle.IDLE_LINE_COLOR
            border_color = OverlayStyle.IDLE_BORDER_COLOR
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            # 3. 가운데 드래그 시 -> 이동 모드
                ...
                self.dragging = True
                # 클릭 시점의 전역 좌표와 창 좌상단 좌표의 차이(Offset) 저장
