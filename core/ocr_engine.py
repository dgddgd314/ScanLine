...
class OCRThread(QThread):
    ...
    def __init__(self, image_queue):
        ...
        self.save_path = None   # 기본 저장 위치
        self.fallback_path = os.path.abspath("output.txt")  # 기본적으로 실행 파일이 있는 위치에 output.txt로 저장

    @pyqtSlot(str)
    def set_save_path(self, path):
        """제어판에서 경로를 변경할 때마다 업데이트"""
        self.save_path = path
        ...
        while self.is_running:
            try:
                ...
                if extracted_text:
                    ...
                    target_path = self.save_path if self.save_path else self.fallback_path  # 저장할 경로 결정
                    ...
                    try:
                        ...
                        self.log_signal.emit(f"⚠️ [경고] 설정된 경로({target_path})에 저장 실패: {primary_error}")
